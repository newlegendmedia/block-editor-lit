...
export class ContentStore extends ResourceStore<ContentId, Content> {
  ...
  async set(content: Content, parentId?: ContentId): Promise<void> {
    ;
    return super.set(content, parentId);
  }
  ...
  async create(modelInfo: ModelInfo, modelDefinition: Model, content: any, parentId?: ContentId): Promise<Content> {
    const id = generateId(modelInfo.type ? modelInfo.type.slice(0, 3).toUpperCase() : '') as ContentId;
    const newContent: Content = { id, modelInfo, modelDefinition, content };
    await this.addCompositeContent(newContent, parentId);
    return newContent;
  }

  async update(id: ContentId, updater: (content: Content) => Content): Promise<Content | undefined> {
    const content = await this.get(id);
    if (content) {
      const updatedContent = updater(content);
      const parentNode = this.tree.parent(id);
      await this.updateCompositeContent(updatedContent, parentNode?.id);
