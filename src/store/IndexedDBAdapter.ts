...
export class IndexedDBAdapter implements StorageAdapter {
  ...
  constructor(dbName: string = 'ContentStore', dbVersion: number = 1, contentStoreName: string = 'content') {
    this.dbName = dbName;
  ...
  async loadContent(id: ContentId): Promise<Content | undefined> {
    try {
      const result = await this.performTransaction('readonly', (store) => store.get(id));
