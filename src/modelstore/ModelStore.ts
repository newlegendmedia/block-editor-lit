	...
	if (isObject(model)) {
	  ...
	  const resolvedProperties = await Promise.all(
		model.properties.map(async (prop, index) => {
		  ...
		  ;
		  return this.deepResolveModel(schemaName, prop, propPath, new Set(resolvedRefs), depth + 1);
		})
	  );
	  ;
	  ...
	  if (Array.isArray(model.itemTypes)) {
		resolvedItemTypes = await Promise.all(
		  model.itemTypes.map((item, index) => {
			...
			;
			return this.deepResolveModel(schemaName, item, itemPath, new Set(resolvedRefs), depth + 1);
		  })
		);
	  } else {
