...
export class DocumentsViewer extends LitElement {
  ...
  private renderDocumentItem(doc: Document) {
    return html`
      <li class="document-item">
        <span
          class="document-title ${doc.id === this.activeDocumentId
            ? "active"
            : ""}"
  ...
  private async openDocument(id: string) {
    ...
    await this.loadDocuments();
    this.dispatchEvent(
      new CustomEvent("document-opened", { detail: { documentId: id } }),
    );
  }
  ...
  private async closeDocument(id: string) {
    ...
    await this.loadDocuments();
    this.dispatchEvent(
      new CustomEvent("document-closed", { detail: { documentId: id } }),
    );
  }
  ...
  private async deleteDocument(id: string) {
    ...
    await this.loadDocuments();
    this.dispatchEvent(
      new CustomEvent("document-deleted", { detail: { documentId: id } }),
    );
  }
}
