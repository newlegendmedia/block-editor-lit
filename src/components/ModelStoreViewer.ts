...
export class ModelStoreViewer extends LitElement {
  ...
  static styles = css`
    ...
    .tree-node {
      padding-left: 20px; /* Fixed indentation for all levels */
    }
    .tree-node-content {
      display: flex;
      align-items: center;
  ...
  connectedCallback() {
    ...
    this.unsubscribe = modelStore.subscribeAll(
  ...
  private renderTreeNode(node: HierarchicalItem<Model>): TemplateResult {
