import {
  ...
  export class UnifiedLibrary {
	...
	private loadDefinitions(type: PropertyType, data: Record<string, Property | Model>): void {
	  Object.entries(data).forEach(([key, value]) => {
		if (!value.type) {
		  ...
		  this.errors.push(`Type mismatch for definition ${key}: expected ${type}, got ${value.type}`);
		} else {
	...
	getDefinition(key: string, type?: PropertyType): Property | Model | undefined {
	  ...
	  if (!item) {
		this.errors.push(`Model Library: Definition not found: { type: '${type}' } with { key: '${key}' }`);
		return undefined;
	  }
	  if (type && item.type !== type) {
		this.errors.push(`Model Library: Type mismatch for ${key}: expected ${type}, got ${item.type}`);
		return undefined;
	  }
  ...
  export function resolveRefs(
	...
	if (isPropertyReference(property)) {
	  if (resolvedRefs.has(property.ref)) {
		library.getErrors().push(`Model Library: ResolveRefs - Circular reference detected: ${property.ref}`);
		return property;
	  }
	  ...
	  if (resolved) {
		...
		library.getErrors().push(`Model Library: ResolveRefs - Failed to resolve ref: ${property.ref}`);
		return property;
	  }
	}
	...
	if (isGroup(property)) {
	  ...
	  if (Array.isArray(property.itemTypes)) {
		resolvedItemTypes = property.itemTypes.map(item => resolveRefs(item, library, new Set(resolvedRefs)));
	  } else {
		const resolvedRef = resolveRefs(property.itemTypes, library, new Set(resolvedRefs));
		if (isGroup(resolvedRef)) {
		  resolvedItemTypes = Array.isArray(resolvedRef.itemTypes) ? resolvedRef.itemTypes : [resolvedRef.itemTypes];
		} else {
		  throw new Error(`Model Library: ResolveRefs - Invalid itemTypes reference: ${JSON.stringify(property.itemTypes)}. Must refer to a group.`);
		}
	  }
	  return {
		...property,
	...
	if (isObject(property)) {
	  return {
		...property,
		properties: property.properties.map(prop => resolveRefs(prop, library, new Set(resolvedRefs)))
	  };
	}
